...
4개 언어(KR/EN/JP/CN) 지원 — 신부가 중국분이라 CN이 필수. 모바일·데스크톱에서 한 번에 한 섹션을
표시하는 스크롤 페이드와 패럴랙스, `prefers-reduced-motion` 대응.
...
- `client/` — Vite + React 18 + TypeScript(strict). 인라인 style로 디자인 프로토타입을 그대로 재현.
  ...
  - `src/motion.tsx` — 모션 유틸: ScrollScene(현재 구간의 섹션 하나만 표시하고 경계에서 페이드) /
    Reveal(딜레이·from 변형 지원, ScrollScene 안에서는 중첩 효과 생략) / useInView /
    useParallax / useCountUp(D+ 카운트업). reduced-motion 대응.
...
- 한 번에 한 섹션 스크롤: `motion.tsx`의 `ScrollScene`은 너비 제한 없이 동작 줄이기가 꺼져 있을 때
  적용한다. `App.tsx`에서 히어로·인사말·사진/囍 장식·두 사람 소개·혼인일·방명록·맺음말을 각각 감싸며,
  `Story.tsx`에서는 각 장의 라벨·삽화·장 제목·본문을 하나의 장면에 포함한다. 장 사이에 독립적으로
  남는 라벨·구분선을 두지 않는다. 상단 언어·음악 바는 장면 밖에 둔다.
  `.scroll-scene`은 상단 바를 제외한 화면 높이 이상의 구간을 갖는다(`100svh`, 미지원 시 `100vh`).
  읽기 영역 중앙에 해당하는 장면 하나만 표시하며 경계에서 이전 장면이 사라진 뒤 다음 장면이 나타난다.
  스크롤을 되돌리면 이전 장면이 다시 표시된다. 짧은 내용은 내부 콘텐츠의 위치를 보정해 화면 중앙에
  머물게 하고, 화면보다 긴 내용과 `data-scene-interactive` 방명록은 문서 흐름대로 스크롤한다.
  장면 내부 `Reveal`은 지연·이동을 생략하며 히어로도 공용 장면 전환을 사용한다.
  비활성 콘텐츠는 포인터 입력을 받지 않지만 본문을 접근성 트리에서 제거하지 않는다. 방명록에 초점이
  있고 해당 장면이 화면에 남아 있으면 불투명도를 1로 유지한다. 입력 중에는 `--focused-scene-height`로
  구간 높이를 유지하고, 초점 이동은 다음 프레임에 확인해 입력칸 사이 이동 중 해제되지 않게 한다.
  모든 장면은 공용 requestAnimationFrame으로 갱신하며 스크롤·창 크기·콘텐츠 높이·초점 변화를 감지한다.
  네이티브 스크롤을 유지하며 강제 스냅·스크롤 탈취·별도 내부 스크롤을 추가하지 않는다.
  동작 줄이기에서는 최소 화면 높이와 중앙 보정을 해제하고 일반 문서 흐름을 사용한다. `index.css`의
  `[data-reveal]`·`[data-scroll-scene]`·`[data-scene-content]` 규칙으로 내용을 즉시 표시한다.
