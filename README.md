...
- 한 번에 한 섹션 — 모바일과 데스크톱 모두 화면 중앙에 해당하는 섹션 하나만 표시합니다. 첫 화면부터 이야기 각 장·방명록·맺음말까지 독립적인 스크롤 구간을 가지며, 경계에서 이전 내용이 사라진 뒤 다음 내용이 나타납니다. 위로 스크롤하면 이전 섹션을 다시 볼 수 있습니다.
- 짧은 내용은 화면 중앙에 머물고, 화면보다 긴 본문과 방명록은 문서 흐름대로 스크롤합니다. 이야기의 라벨·삽화·장 제목·본문은 함께 전환됩니다. 방명록에 초점이 있고 해당 섹션이 화면에 남아 있는 동안에는 입력부를 선명하게 유지하며, 입력 중에는 섹션 높이를 유지해 키보드로 인한 위치 변화를 줄입니다.
- 네이티브 스크롤을 유지하며 강제 스냅이나 별도 내부 스크롤을 사용하지 않습니다. 상단 언어·음악 버튼은 항상 사용할 수 있습니다. `prefers-reduced-motion`에서는 섹션 전환과 화면 중앙 보정을 해제하고 모든 내용을 일반 문서 흐름으로 즉시 표시합니다.
