...
- 모바일 스크롤 페이드 — 화면 너비 600px 이하에서 인사말·사진/장식·두 사람 소개·이야기 각 장·혼인일·맺음말이 스크롤 위치에 따라 나타나고 사라지며, 위로 돌아가면 다시 표시됩니다. 이야기의 삽화·장 제목·본문은 한 묶음으로 움직이고, 긴 본문은 읽는 동안 선명하게 유지됩니다.
- 데스크톱은 기존 1회 리빌과 패럴랙스를 유지합니다. 히어로는 기존 스크롤 퇴장 효과를 사용하며, 방명록은 반복 페이드에서 제외합니다. 네이티브 스크롤을 유지하고 `prefers-reduced-motion`에서는 내용이 즉시 표시됩니다.
