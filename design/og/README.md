...
- 2026-09-12에 시안 a를 `client/public/og.jpg`로 적용했다. 혼인일 전후 모두 자연스럽도록 시제 없는 문구를 골랐다.
...
cp design/og/og-a.jpg client/public/og.jpg
```

문구·크기·색은 `make_og.py`의 `variants`와 `INK`/`MUTE`에서 바꾼다. 적용 뒤 재빌드하고
카카오 캐시 초기화(developers.kakao.com/tool/debugger/sharing)가 필요하다.
