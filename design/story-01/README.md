...
종이색 배경의 1296×1056 이미지에 432×352 프레임 9개가 3×3으로 정렬되어 있다.
...
출력은 `client/src/assets/story-01-sprite.png`와 `.webp`다.
...
docker run --rm --user 0 -v "$PWD:/src" \
  ...
  /src/design/story-01/prepare.cjs
```

`/srv/drop/files/story-01-language-app.zip`은 이전 투명 배경 시안의 원본 패키지이며,
