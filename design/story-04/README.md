...
`useInView(0.5)`로 지도의 절반이 화면에 들어온 것을 감지하면 `client/src/index.css`의
`.story-map` 애니메이션을 시작한다. Q 명령으로 만든 2차 베지어 곡선 네 구간이 약 3.8초에 이어지고,
완성된 경로를 잠시 유지한 뒤 서서히 지우며 6초 주기로 반복한다. 선은 각 구간에서 일정한 속도로 그린다.
진입 전에는 모든 선과 점을 숨긴다. 출발점은 첫 선이 시작될 때 나타나고, 이후 각 도착점은 선이 완성된
시점부터 0.12초 동안 나타난다. 갈색 테두리와 적갈색 중심점을 하나의 SVG 그룹으로 함께 페이드한다.
`prefers-reduced-motion`에서는 모든 선과 점을 완성된 상태로 고정한다.
곡선에는 고정된 SVG 노이즈·변위 필터와 불투명도 마스크로 가장자리의 미세한 흔들림과 종이 결에 따른
잉크 농도 차이를 더한다. 질감이 더해져도 선명하게 보이도록 선 두께는 SVG 기준 3.8로 조정했다.
필터 자체는 움직이지 않으며, 기존 경로 순서와 구도는 유지한다. 마지막 도착점은 그려진 해안선에 맞췄다.
